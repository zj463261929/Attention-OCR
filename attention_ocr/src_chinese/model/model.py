...
fw_correct = codecs.open("/opt/zhangjing/ocr/Attention-OCR-chinese-version/result/correct.txt", 'w', "utf-8")
fw_imcorrect = codecs.open("/opt/zhangjing/ocr/Attention-OCR-chinese-version/result/imcorrect.txt", 'w', "utf-8")

fw_data = codecs.open("/opt/zhangjing/ocr/Attention-OCR-chinese-version/result/data.txt", 'w', "utf-8")
def chinese(c):	
	c = hex(c) #将10进制转16， 如0x75c5L，病
	c = c[2:len(c)-1]
	c = "\u" + c 	
	#print ( c.decode("raw_unicode_escape").encode("utf-8") )		
	return c
	
def chinese_display(output):
    lst = []
    for c in output:
        c = c + 19968 - 3
        c = hex(c) #将10进制转16， 如0x75c5L，病
...
class Model(object):
    ...
    def chinese_display(self, filename, output, fw, flag):
        ...
        for c in output:
            if c > 2 and c < 241+3 :#4450:#24:#951: 
    ...
    def launch(self):
        ...
        #加载字典
        #trieTree.trie = trieTree.construction_trietree("/opt/zhangjing/ocr/Attention-OCR-chinese-version/dict.txt", "/opt/zhangjing/ocr/Attention-OCR-new-version/lm.binary") #("userdic.txt")
        ...
        if self.phase == 'test':
            ...
            for batch in self.s_gen.gen(self.batch_size):
                ...
                for idx, output, ground in zip(range(len(grounds)), step_outputs, grounds):
                    ...
                    '''       
                    #5.31 添加字典矫正，准确率可提供10%                                                     
                    s = ''.join([chr(c-13+97) if c-13+97>96 else chr(c-3+48) for c in output_valid])
                    print ("recongition word:{}".format(s))
                     
                    output_valid[:] = []
                    #输出矫正后的word        
                    if trieTree.trie is not None:                         
                        word = trieTree.correct_word(s, 3, trieTree.trie) 
                        print ("correct word:{}".format(word))
                        for c in word:
                            output_valid.append( ord(c) - 97 + 13 if ord(c) > 96 else ord(c) - 48 + 3 )
                    ...
                    if num_incorrect < 1:
                        ...
                        #print ("ground:{}".format( self.chinese_display(ground_valid, fw_correct, 1)) )
                        #print ("output:{}".format( self.chinese_display(output_valid, fw_correct, 0)) )
                    else:
                        self.chinese_display(file_list, ground_valid, fw_imcorrect, 1)
                        self.chinese_display(file_list, output_valid, fw_imcorrect, 0)
                        #print ("ground:{}".format( self.chinese_display(ground_valid, fw_imcorrect, 1)) )
                        #print ("output:{}".format( self.chinese_display(output_valid, fw_imcorrect, 0)) )
                    
                    if num_incorrect < 1:
                        num_correct = num_correct + 1
                logging.info('%f out of %d correct, precision=%f' %(num_correct, num_total, num_correct/(num_total+1)))
